...
int dtls_srtp_init(DtlsSrtp* dtls_srtp, DtlsSrtpRole role, void* user_data) {
  ...
  if (setup_ret != 0) {
    LOGE("mbedtls_ssl_setup failed: %d", setup_ret);
  }
...
static int dtls_srtp_key_derivation(DtlsSrtp* dtls_srtp, const unsigned char* master_secret, size_t secret_len, const unsigned char* randbytes, size_t randbytes_len, mbedtls_tls_prf_types tls_prf_type) {
  ...
  /* PATCH(libpeer): const-correctness — client/server_{key,salt} above are
   * const uint8_t*, so the local/remote aliases must also be const to
   * avoid -Werror=discarded-qualifiers under GCC 14. The values are
   * read-only handed to srtp_policy_t which takes them as unsigned
   * char* but treats them as input. */
...
static int dtls_srtp_do_handshake(DtlsSrtp* dtls_srtp) {
  ...
  // NOTE: shared across calls — fine while there's at most one DtlsSrtp
  // session in flight (today's reality). If we ever run two concurrent
  // sessions this needs to move into DtlsSrtp.
